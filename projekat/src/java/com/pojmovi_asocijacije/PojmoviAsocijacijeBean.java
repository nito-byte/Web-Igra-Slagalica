...
@ManagedBean(name = "PojmoviAsocijacijeBean")
@SessionScoped
public class PojmoviAsocijacijeBean {
    
    public static  int sifraTrenutne=0;
...
//    }
    ...
    public String cadastrarSinonimiA() {
        ...
        for (Word item : words) {
            ...
            usuario.setSifra(sifra);
            inserir.persist(usuario);
            word=null;
            words=null;
        }
        return "/supervizor/unosAsocijacijeSinonimiB.xhtml?faces-redirect=true";
    }
    
    public String cadastrarSinonimiB() {

        for (Word item : words) {
            SinonimiBDAO inserir = new SinonimiBDAOImpl();
            SinonimiB usuario = new SinonimiB();
            usuario.setKolonaB(item.getRec());
            usuario.setSifra(sifra);
            inserir.persist(usuario);
            word=null;
            words=null;
        }
        return "/supervizor/unosAsocijacijeSinonimiV.xhtml?faces-redirect=true";
    }
